...
async function loadOpenTables() {
  ...
  snapshot.forEach(doc => {
    ...
    tableCard.innerHTML = `
      ...
      <div style="display:flex; gap:5px; flex-wrap:wrap;">
        <button onclick="selectTable('${table.tableNumber}')">Apri</button>
...
async function showTableHistory(tableNumber) {
  ...
  let historyText = `TAVOLO ${tableNumber}\nTotale: €${table.total.toFixed(2)}\n\n`;

  orders.forEach((order, index) => {
    historyText += `ORDINE ${index + 1} - ${order.type === "add" ? "Aggiunta" : "Nuova comanda"}\n`;

    order.items.forEach(item => {
      historyText += `${item.quantity} x ${item.name} - €${(item.price * item.quantity).toFixed(2)}\n`;

      if (item.modification) {
        historyText += `   Modifica: ${item.modification}\n`;
      }
    });

    if (order.notes) {
      historyText += `Note: ${order.notes}\n`;
    }

    historyText += `Totale ordine: €${order.total.toFixed(2)}\n\n`;
  });
...
async function closeTable(tableNumber) {
  const confirmClose = confirm(`Chiudere il tavolo ${tableNumber}?`);
